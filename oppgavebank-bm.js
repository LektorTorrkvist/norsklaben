/* ══════════════════════════════════════════════════════
   OPPGAVEBANK – bokmål
   Tom bank + komplett motor for adaptiv mengdetrening
══════════════════════════════════════════════════════ */

const MT_BANK = [
  /* Nye øvelser legges til her */
]; // end MT_BANK
...
function mtGetTilgjengeligeOppgaver() {
...
function mtUpdateAntalMeta() {
  ...
  if (!inp) return;
  var tilgjengelig = mtGetTilgjengeligeOppgaver();
  var maks = Math.min(25, tilgjengelig || 25);
  inp.max = String(maks);
  var val = parseInt(inp.value, 10);
  if (Number.isFinite(val) && val > maks) inp.value = String(maks);
  if (hint) {
    if (tilgjengelig === 0) {
      hint.textContent = 'Velg minst \u00E9n kategori for \u00E5 se hvor mange oppgaver som er tilgjengelige.';
    } else {
      hint.textContent = 'Tilgjengelige med valgene dine: ' + tilgjengelig + '. Du kan starte med opptil ' + maks + '.';
    }
  }
}
...
function mtStart() {
  mtShuffleBank();
  var valgte = [].slice.call(document.querySelectorAll('.mt-kat-btn[data-sel="1"]')).map(function (b) { return b.dataset.kat; });
  if (!valgte.length) { alert('Velg minst \u00E9n kategori for \u00E5 starte.'); return; }
  ...
  if (pool.length && grunnTal > maksTillate) {
    alert('Du ba om ' + grunnTal + ' oppgaver, men med disse valgene er maks ' + maksTillate + '. Starter med ' + maksTillate + '.');
  }
...
/* ══════════════════════════════════════════════════════
   RENDER – bygg HTML for hver oppgavetype
══════════════════════════════════════════════════════ */
function mtRenderTask() {
  ...
  var vLbl = { lett: 'Lett', medium: 'Medium', vanskeleg: 'Vanskelig' }[t.vanske] || '';
  ...
  /* ── klikk_marker ── */
    ...
    inputHTML = '<div style="margin-top:0.8rem">' +
      '<p style="font-size:12px;color:#8a8a84;margin-bottom:0.5rem">Klikk p\u00E5 ordene du mener er <strong>' + mtEsc(t.maalordklasse) + '</strong>. Klikk igjen for \u00E5 fjerne markeringen.</p>' +
      ...
      '<button onclick="kmSjekk()" style="background:#e5822a;color:#fff;border:none;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:14px;font-weight:500;padding:8px 18px;cursor:pointer">Sjekk markeringer</button> ' +
  ...
  /* ── fix (rett feil i tekst) ── */
    ...
    inputHTML = '<div style="margin-top:0.8rem">' +
      '<p style="font-size:12px;color:#8a8a84;margin-bottom:0.5rem">Rett feilene direkte i teksten under.</p>' +
  ...
  /* ── drag_ord ── */
    ...
    inputHTML = '<div style="margin-top:0.8rem">' +
      ...
      '<span id="mtdo-placeholder" style="font-size:13px;color:#aaa">Trykk p\u00E5 ordene over i riktig rekkef\u00F8lge\u2026</span></div>' +
  ...
  /* ── avsnitt_klikk ── */
    ...
    inputHTML = '<div style="margin-top:0.8rem">' +
      '<p style="font-size:12px;color:#8a8a84;margin-bottom:0.5rem">Klikk p\u00E5 det <strong>f\u00F8rste ordet</strong> i hver setning der du vil starte et nytt avsnitt.</p>' +
  ...
  var nextLabel = MTS.idx + 1 < MTS.tasks.length ? 'Neste oppgave \u2192' : 'Se resultat \u2192';
  $mt('mt-task-wrap').innerHTML =
...
function mtFinish(correct, chosen, t) {
  ...
  var html = '<strong>' + (correct ? '\u2713 Riktig!' : '\u2717 Feil') + '</strong>';
  if (!correct && t.fasit) html += ' Riktig svar: <strong>' + mtEsc(t.fasit) + '</strong>';
...
/* ══════════════════════════════════════════════════════
   NAVIGASJON – neste oppgave / oppsummering
══════════════════════════════════════════════════════ */
function mtNext() {
  if (!MTS.answered) {
    var t = MTS.tasks[MTS.idx];
...
function mtShowSummary() {
  ...
  var msgs = [[90, 'Fantastisk! Du behersker dette veldig godt.'], [70, 'Bra jobba! Du er p\u00E5 god vei.'], [50, 'Greit! \u00D8v litt mer p\u00E5 de vanskelige oppgavene.'], [0, 'Ikke gi opp \u2013 pr\u00F8v igjen!']];
  ...
  if (hist) {
    ...
    MTS.tasks.forEach(function (t, i) {
      ...
      html += '<div style="' + bg + ';border:1px solid;border-radius:8px;padding:0.5rem 0.8rem;margin-bottom:6px;display:flex;gap:10px;align-items:flex-start;font-size:13px">' +
        ...
        '<div style="color:#1a1a18;flex:1">' + mtEsc((t.q || t.sporsmal || '').substring(0, 80)) + (ok ? '' : ' <span style="color:#f0a090">\u2192 Riktig: ' + mtEsc(t.fasit) + '</span>') + '</div>' +
        '</div>';
    });
    hist.innerHTML = html;
  }
