...
var nlCategoryData = [
  // Gruppe 1 – Grunnleggjande grammatikk (7 kategoriar)
  [
    {n: 'Og / Å',             d: 'Skilnaden mellom bindeordet «og» og infinitivsmerket «å».'},
    {n: 'Bindeord',           d: 'Bruk bindeord til å binde setningar og avsnitt saman.'},
    {n: 'Konsonantdobling',   d: 'Kort vokal = dobbel konsonant. Lang vokal = enkel.'},
    {n: 'Teiknsetting',       d: 'Komma, kolon, semikolon og hermeteikn – når brukar du kva?'},
    {n: 'Særskriving',        d: 'Skal det vere eitt ord eller to? Reglane for samansetjing.'},
    {n: 'Ordklassar',         d: 'Substantiv, verb, adjektiv – kjenner du att ordklassane?'},
    {n: 'Forstå oppgåva',     d: 'Kva vil eigentleg oppgåva? Nøkkelord og oppgåveord.'}
  ],
  // Gruppe 2 – Setning og avsnitt (5 kategoriar)
  [
    {n: 'Setningsbygging',    d: 'Klipp løpande setningar, varier opningar, slå saman og analyser rytme.'},
    {n: 'Temasetning',        d: 'Kvar avsnitt treng ei temasetning – skriv og finn dei.'},
    {n: 'Årsak og verknad',   d: '«Fordi», «difor», «som følgje av» – bygg logiske kjeder.'},
    {n: 'Referansebinding',   d: 'Unngå å gjenta same ord – varier referansane i teksten.'},
    {n: 'Disposisjon',        d: 'Planlegg teksten – sett avsnitt i rett rekkjefølgje.'}
  ],
  // Gruppe 3 – Tekst og sjanger (5 kategoriar)
  [
    {n: 'Tekststruktur',         d: 'Ingress, hovuddel og avslutting – kva høyrer heime kvar?'},
    {n: 'Sjanger og register',   d: 'Fagartikkel, blogg eller kåseri – kjenner du sjangrane?'},
    {n: 'Drøfting',              d: 'Sjå begge sider, veg argument og trekk ei slutning.'},
    {n: 'Overskrift og ingress', d: 'Ein god overskrift fangar lesaren. Korleis lagar du ein?'},
    {n: 'Tekstopning',           d: 'Unngå klisjear – opne teksten med noko som grip tak.'}
  ],
  // Gruppe 4 – Kjeldebruk (4 kategoriar)
  [
    {n: 'Kjeldebruk',        d: 'Korrekt format på kjeldeliste og kjeldehenvisningar i teksten.'},
    {n: 'Parafrase',         d: 'Skriv om med eigne ord – ikkje kopier frå kjelda.'},
    {n: 'Sitat',             d: 'Korleis integrerer du eit sitat grammatisk og korrekt?'},
    {n: 'Tal og statistikk', d: 'Gjer tal forståelege og skriv om usikre statistikkar.'}
  ],
  // Gruppe 5 – Språk og stil (3 kategoriar)
  [
    {n: 'Ordval og presisjon',  d: 'Vel presise ord – ikkje berre «farleg», «problem», «mange ting».'},
    {n: 'Bruke eksempel',       d: 'Gode eksempel gjer meiningane dine tydelege og truverdige.'},
    {n: 'Tilpass til lesaren',  d: 'Forklar fagord og uttrykk slik at lesaren forstår.'}
  ]
];

var nlCategoryIdsByGroup = [
  ['og-aa', 'bindeord-overgangar', 'dobbel-konsonant', 'teiknsetting', 'saerskriving-samansetjing', 'ordklasse', 'oppgaveforstaing'],
  ['setningsbygging', 'temasetning', 'aarsak-samanheng', 'referansekjede', 'logisk-struktur'],
...
function nlLoadTekstanalyseWidget() {
  ...
  script.onerror = function() {
    ...
    if (fallback) {
      ...
      fallback.innerHTML = '<strong>API-et svarar ikkje enno.</strong><span>Start den lokale tenesta på <code>' + nlMtEscHtml(api) + '</code> eller opne sida med <code>?api=adresse</code> for å peike til rett server.</span>';
    }
    if (window.console && console.warn) {
      console.warn('Tekstanalyse API ikkje tilgjengeleg på ' + api);
    }
  };
...
function nlMtPickPrompt(task) {
  if (!task) return 'Oppgåve';
  return String(task.q || task.sporsmal || 'Oppgåve');
}

function nlMtCleanPromptForTitle(prompt) {
  var text = String(prompt == null ? '' : prompt).trim();
  if (!text) return 'Oppgåve';

  // Remove verbose/imported lead-ins so titles are short and student-facing.
  text = text
    .replace(/^skriv\s+ei\s+informasjon\s+på\s+tre\s+ulike\s+m[aå]tar\s*[-–:]+\s*/i, '')
    .replace(/^her\s+kan\s+du\s+l[aæ]re\s+meir\s*[-–:]+\s*/i, '')
  ...
  return text || String(prompt).trim() || 'Oppgåve';
}
...
function nlMtBuildFasitMetaHtml(task) {
  ...
  if (task && task.eksempel_svak) bits.push('<div class="fr"><strong>Kan bli betre:</strong> ' + nlMtEscOrEmpty(task.eksempel_svak) + '</div>');
...
function nlStripTaskTypePrefix(text) {
  ...
  if (!out) return out;
  return out.replace(/^\s*(?:feilretting|fylloppg[aå]ve|fyll\s+inn\s+rett\s+ord|sorter(?:ing)?|rangere|rangering|identifisering|analyse|bygg(?:je|e)?(?:oppg[aå]ve)?|omskriving|korrigering|rekkjef[øo]lgjeoppg[aå]ve)\s*:\s*/i, '').trim();
}
...
function nlMtBuildExercise(task, i, localIx) {
  ...
  if (!html) return '';
  // Inject data-attrs so the adaptive engine can read regel/eks/fasit reliably
...
function nlMtBuildExerciseCore(task, i, localIx) {
  ...
  if (type === 'mc') {
    ...
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Vel eitt alternativ.</div>' +
      guideHtml +
  ...
  if (type === 'cloze') {
    ...
    return '<article class="ei">' + header +
      ...
      '<div class="inst">Skriv ordet som manglar.</div>' +
      guideHtml +
  ...
  if (type === 'open') {
    ...
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Skriv 1-3 setningar.</div>' +
      guideHtml +
  ...
  if (type === 'drag_ord') {
    ...
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Set orda i rett rekkjefølgje.</div>' +
      guideHtml +
  ...
  if (type === 'drag_kolonne') {
    ...
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Dra kvart element til rett kolonne.</div>' +
      guideHtml +
  ...
  if (type === 'fix') {
    ...
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Rett feila direkte i teksten.</div>' +
      guideHtml +
  ...
  if (type === 'rank') {
    ...
    return '<article class="ei">' + header +
      ...
      '<div class="ex-controls"><button class="btn-check" data-check="rank" data-target="' + rankId + '" data-score="' + scoreRankId + '">Sjekk rekkjefølgje</button><button class="btn-reset" data-reset="rank" data-target="' + rankId + '" data-score="' + scoreRankId + '">Bland på nytt</button><span id="' + scoreRankId + '" class="ex-score"></span></div>' +
      '<button class="btn-fasit" data-fasit="fb-' + uniq + '">' + revealLabel + '</button>' +
      '<div class="fasit-box" id="fb-' + uniq + '"><div class="fb"><div class="fl">' + revealTitle + '</div><p class="fb-ans">' + revealBody + '</p>' + metaHtml + '</div></div>' +
      '</div></article>';
  }
  ...
  if (type === 'fillsel') {
    ...
    var fillselHtml = sentences.map(function(s, si) {
      ...
      return '<p class="fillsel-sentence">' + nlMtEscHtml(s.pre || '') +
        '<select class="fill-select" data-answer="' + nlMtEscHtml(answer) + '"><option value="">– vel –</option>' + opts + '</select>' +
        nlMtEscHtml(s.post || '') + '</p>';
    }).join('');
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Vel rett alternativ i kvar rullegardin.</div>' +
      guideHtml +
  ...
  if (type === 'mcset') {
    ...
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Vel eitt alternativ på kvart spørsmål.</div>' +
      guideHtml +
  ...
  if (type === 'burger_sort') {
    ...
    return '<article class="ei">' + header +
      '<div class="ec">' +
      promptBoxHtml +
      '<div class="inst">Dra kvart avsnitt til rett lag.</div>' +
      guideHtml +
...
function nlImportMTBankTasks() {
  ...
  /* Always clear legacy inline tasks first: card dropdowns should be BANKV2-driven only. */
  ...
  if (imported > 0 && window.console && console.info) {
    console.info('[Skrivelab] Importerte', imported, 'oppgåver frå BANKV2.');
  }
...
function nlStripLegacyInlineTasks() {
  /* Hard reset of all card content: BANKV2 is the only source of tasks. */
...
function nlCheckFill(cid, sid) {
  ...
  if (total === 0) {
    nlSetScore(sid, 'Ingen autosjekk i denne oppgåva. Bruk "Vis fasit" for samanlikning.');
    return;
  }
...
function nlCheckMc(tid, sid) {
  ...
  if (!selected) {
    nlSetScore(sid, 'Vel eitt alternativ før du sjekkar.');
    return;
  }
...
function nlCheckMcSet(tid, sid) {
  ...
  if (!areas.length) {
    nlSetScore(sid, 'Ingen autosjekk i denne oppgåva.');
    return;
  }
  ...
  if (unanswered > 0) {
    nlSetScore(sid, 'Vel eitt alternativ på alle delspørsmål.');
    return;
  }
...
function nlCheckDragOrd(tid, sid) {
  ...
  if (!chosen.length) {
    nlSetScore(sid, 'Bygg setninga før du sjekkar.');
    return;
  }
  ...
  if (exact) {
    ...
    nlSetScore(sid, 'Rett rekkjefølgje.', 'ok');
    return;
  }
...
function nlCheckFix(tid, sid) {
  ...
  if (total === 0) {
    ...
    if (!now || now === original) nlSetScore(sid, 'Gjer endringar i teksten før du sjekkar.');
    else nlSetScore(sid, 'Svar registrert. Denne oppgåva har open vurdering.', 'ok');
  } else {
...
function nlCheckMark(tid, sid) {
  ...
  if (!selected.length) {
    nlSetScore(sid, 'Klikk og marker minst eitt svar før du sjekkar.');
    return;
  }
  ...
  if (wrong === 0 && missed === 0) {
    ...
    nlSetScore(sid, 'Rette: ' + hits + '. Feilmarkert: ' + wrong + '. Manglar: ' + missed + '.', 'err');
  }
}
...
function nlInitWriteChecks() {
  // Langsvar skal vurderast med fasit/eigenvurdering, ikkje autosjekk.
  return;
...
function nlInitContextualWritePlaceholders() {
  document.querySelectorAll('.ei').forEach(function(ei) {
    ei.querySelectorAll('.write-area').forEach(function(ta) {
      ...
      if (ph && ph !== 'skriv her...' && ph !== 'skriv her') return;
...
function nlKeyTermsFromFasit(fasitText) {
  var stop = {
    'som':1, 'det':1, 'den':1, 'dei':1, 'for':1, 'med':1, 'til':1,
    'og':1, 'er':1, 'var':1, 'kan':1, 'skal':1, 'ikkje':1, 'eller':1,
    'på':1, 'av':1, 'om':1, 'ein':1, 'ei':1, 'eit':1, 'har':1, 'vil':1
  };
  var freq = {};
...
function nlCheckWrite(tid, sid) {
  ...
  if (!text) {
    nlSetScore(sid, 'Skriv eit svar før du sjekkar.');
    return;
  }

  var words = text.split(/\s+/).filter(Boolean).length;
  if (words < 4) {
    nlSetScore(sid, 'For kort svar enno. Prøv minst 1-2 heile setningar.', 'err');
    return;
  }
  ...
  if (words >= 30 && ratio >= 0.45) {
    nlSetScore(sid, 'Sterkt utkast. Du treff fleire sentrale punkt (' + hits + '/' + keyTerms.length + ').', 'ok');
    return;
  }
  ...
  nlSetScore(sid, 'Delvis treff. Samanlikn med fasit og bygg ut med fleire fagord/punkt. (' + hits + '/' + keyTerms.length + ')', 'err');
}
...
function nlCheckSort(eid, sid) {
  ...
  if (inBank > 0) {
    nlSetScore(sid, 'Plasser alle orda i ei bøtte før du sjekkar (' + inBank + ' att).');
    return;
  }
  var answers = {};
...
function nlBuildBurger(eid) {
  ...
  bankLbl.textContent = 'Kort som skal sorterast:';
...
function nlCheckBurger(eid, sid) {
  ...
  if (inBank > 0) {
    nlSetScore(sid, 'Plasser alle korta i eit lag før du sjekkar (' + inBank + ' att).');
    return;
  }
...
function nlTypeMetaFromCheckType(checkType) {
  ...
  if (t === 'fill') return { cls: 'of', label: 'Fylloppgåve' };
...
/* ── NEW MANUAL FLOW (v2 oppgåvebank-motor) ── */
...
function nlShowWelcomeModal() {
  ...
  if (!hasHistory) {
    ...
    if (titleEl) titleEl.textContent = 'Velkomen til Skrivelab!';
    if (msgEl) msgEl.textContent = 'Her kan du øve på norsk skriving \u2013 grammatikk, rettskriving, tekststruktur og meir. Vel kategoriar, trykk \u00abStart \u00f8ving\u00bb, og ten XP for kvart rett svar!';
  } else {
...
function nlAdDifficulty(ei) {
  if (ei.querySelector('.b.da')) return 'vidarekomande';
...
function nlAdTypeFromExercise(ei) {
  ...
  return badge ? badge.textContent.trim() : 'Oppgåvetype';
}
...
function nlAdBuildList(cats, level, count) {
  ...
  var hard = all.filter(function(ei) { return nlAdDifficulty(ei) === 'vidarekomande'; });
...
function nlAdUpdateBadges(current) {
  ...
  var diffLabel = diff === 'vidarekomande' ? 'Vidarekomande' : diff === 'middels' ? 'Middels' : 'Lett';
  var diffStyle = diff === 'vidarekomande'
...
function nlAdTitleFromExercise(ei) {
  if (!ei) return 'Oppgåve';
  var title = ei.querySelector('.etit');
  if (!title) title = ei.querySelector('h4, h3, h2');
  var text = title ? title.textContent.trim() : 'Oppgåve';
  text = nlStripTaskTypePrefix(text);
  return text || 'Oppgåve';
}

/* ══════ XP visual effects on correct answer ══════ */
var nlAdStreak = 0; // consecutive correct answers in current session
...
function nlAdPulseScore() {
  ...
  void pill.offsetWidth; // reflow to re-trigger
...
function nlAdPlayCorrectEffects(ei, points) {
  ...
  // 1. Flash the exercise green
  nlAdFlashExercise(ei);

  // 2. Float "+XP" from the score pill
  var scorePill = document.getElementById('nl-ad-score-pill');
  nlAdSpawnXpFloat(scorePill, pts, nlAdStreak >= 3);

  // 3. Pulse the score pill
  nlAdPulseScore();

  // 4. Sparks on streaks (3+) and big bursts on 5+
  if (nlAdStreak >= 3) {
...
function nlAdMasteryData(pct) {
  if (pct >= 90) return {
    ...
    comment: 'Du viser sterk meistring av stoffet. Hald fram slik, og utfordre deg sjølv med vanskelegare oppgåver neste gong!'
  };
  if (pct >= 70) return {
    medal: '&#11088;',
    heading: 'Bra jobba!',
    comment: 'Du er på god veg! Sjekk rettingane under for å sjå kva du kan forbetre. Øving gjer meister!'
  };
  if (pct >= 50) return {
    medal: '&#128170;',
    heading: 'Halvvegs der!',
    comment: 'Du har forstått mykje, men det er nokre tema du bør øve meir på. Gå gjennom rettingane og prøv igjen – du kjem til å merke framgang!'
  };
  return {
    medal: '&#127793;',
    heading: 'Ein god start!',
    comment: 'Alle startar ein stad! Sjekk rettingane under nøye og prøv same kategoriane igjen – du vil bli betre for kvar runde.'
  };
}
...
function nlAdShowSummary() {
  ...
  if (rettingEl) {
    if (!corrections.length) {
      rettingEl.innerHTML = '<div class="adp-summary-row ok"><strong>Null feil!</strong> Du klarte alle oppgåvene rett – godt jobba!</div>';
    } else {
      var html = '<h5>Øv på desse igjen</h5>';
      var catCounts = {};
...
function nlAdSetFeedback(text, isCorrect, ruleText, eksText, fasitText) {
  ...
  if (eksText) {
    ...
    if (cleanEks) {
      html += '<div class="adp-fb-eks"><strong>&#128221; Døme:</strong> ' + cleanEks + '</div>';
    }
  }
...
function nlAdSyncActionButtons() {
  ...
  if (isChecked) {
    ...
    nextBtn.textContent = (nlAdState.idx + 1 < nlAdState.list.length) ? 'Neste oppgåve →' : 'Fullfør økt →';
...
function nlAdTriggerCheck() {
  ...
  if (!hasResult) {
    // Show the prompt (e.g. "Vel eitt alternativ") without marking as done
...
function nlAdUpdateStatus() {
  ...
  if (p) p.textContent = 'Oppgåve ' + (idx + 1) + ' av ' + total;
  var pts = 0;
...
function nlAdStart() {
  ...
  if (!cats.length) {
    alert('Vel minst éin kategori før du startar økta.');
    return;
  }
  ...
  if (!list.length) {
    alert('Ingen oppgåver passar vala dine. Prøv fleire kategoriar eller anna nivå.');
    return;
  }
