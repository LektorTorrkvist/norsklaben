...
function nlBankOpenExercise(ei, list, idx) {
  ...
  if (title) title.textContent = et ? et.textContent.trim() : 'Oppgåve';
...
function nlAdPedagogicalMsg(scoreText, isCorrect, ruleText) {
  ...
  if (ratio >= 0) {
    if (ratio >= 0.7) {
      return 'Nesten! Du har forstått det meste — sjå nøye på kva som manglar.';
    } else if (ratio >= 0.4) {
      return hasRule
        ? 'Du er på rett veg! Les regelen under for å forstå kvifor.'
        : 'Du er på rett veg! Samanlikn svaret ditt med fasiten for å forstå kvifor.';
    } else {
      return hasRule
        ? 'Ikkje gje opp — studer regelen under og prøv liknande oppgåver.'
        : 'Ikkje gje opp — samanlikn svaret ditt med fasiten og prøv på nytt.';
    }
  }
  return 'Sjå tilbakemeldinga — det hjelper til neste gong!';
}
