...
function nlAdBuildList(cats, level, count) {
  ...
  cats.forEach(function(cat) {
    Array.prototype.forEach.call(document.querySelectorAll('.main .card[data-cat="' + cat + '"] .ei'), function(ei) {
      if (nlAdIsAdaptiveEligible(ei)) all.push(ei);
    });
  });
...
function nlAdUpdateBadges(current) {
  ...
  var cnEl = card ? card.querySelector('.cn') : null;
  var catName = cnEl ? cnEl.textContent.trim() : '';
  ...
  var diffStyle = diff === 'vidarekomande'
    ...
    : 'background:#edf8f1;color:#1a5f3b;border-color:#bcdac8;';
...
function nlAdShowSummary() {
  ...
  nlAdState.list.forEach(function(ei) {
    ...
    if (!res.correct) corrections.push(res);
  ...
  if (rettingEl) {
    if (!corrections.length) {
      ...
      corrections.forEach(function(item) {
        var cleanMsg = nlAdCleanCorrectionMessage(item.message);
...
function nlAdSyncActionButtons() {
  ...
  var checkEl = current.querySelector('.btn-check');
  var isMC = checkEl && checkEl.dataset.check === 'mc';
  var hasCheck = !!checkEl;
  var isChecked = nlAdState.checked.has(current);

  if (checkBtn) {
    // MC: auto-checks on click → hide button. Others: show as "Sjekk svar".
    checkBtn.style.display = (hasCheck && !isMC) ? '' : 'none';
    checkBtn.textContent = 'Sjekk svar';
  }
  // Next is never blocked — clicking it will auto-check if not yet checked
